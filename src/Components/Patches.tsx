...
import { MonsterGrowl, Lazers } from "../patches/exports";

const synthSchool = [MonsterGrowl, Lazers];
...
const Patches = ({ category, selectPatch }) => {
  return (
    <>
      {category === "Synth School" && (
        <div className="patch-button_container">
          {synthSchool.map((item, index) => {
            return (
              <div key={index} className="patch-button" onClick={() => selectPatch(item)}>
